...
class Attandances_bio_manual extends CI_Model {
    ...
    public function get_bio_att($em_id,$att_type,$curr_month)

    {

     // var_dump(date('Y-m-d',strtotime($_GET['selected_date'])));
    ...
    //  die();
      ...
      //$bio->like('LogDate',date('Y-m-d'));      
	    ...
	    $bio->like('LogDate',date('Y-m-d',strtotime($_GET['selected_date'])));	  

      if($att_type == "in"){

        $bio->order_by('DeviceLogId', 'ASC');/// for in time
