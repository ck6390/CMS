						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									...
									<td>
										<strong><?= htmlspecialchars($list->attribute_name,ENT_QUOTES,'UTF-8') ?></strong>
									</td>

									<td><?= htmlspecialchars($list->attribute_max_score,ENT_QUOTES,'UTF-8') ?></td>
									<td><?= $list->is_active ? anchor("office/{$this->misc->_getClassName()}/deactivate/" . $list->attribute_p_id, '<span class="btn btn-info btn-xs"><i class="fa fa-check"></i> Active</span>') : anchor("office/{$this->misc->_getClassName()}/activate/" . $list->attribute_p_id, '<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Inactive</span>'); ?>
									</td>
									<td>
										<a href="<?php echo site_url("office/{$this->misc->_getClassName()}/edit/{$list->attribute_p_id}"); ?>" class="btn btn-success btn-xs">
											<i class="fa fa-pencil"></i>
										</a>
										<?php if($this->auth->_isDeveloper()) { ?>
											<button class="btn btn-danger btn-xs deleteRow" value="<?= $list->attribute_p_id ?>">
