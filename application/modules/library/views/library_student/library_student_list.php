						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									...
									<td>
										<strong><?= '<span class="badge badge-primary">'.htmlspecialchars($list->student_unique_id,ENT_QUOTES,'UTF-8').'</span><br/>' ?></strong>
									</td>
									<td>
										<strong><?= '<span class="badge badge-primary">'.htmlspecialchars("1".$list->student_unique_id,ENT_QUOTES,'UTF-8').'</span><br/>' ?></strong>
									</td>
									...
									<td><?php echo ($list->is_active) ? anchor("{$this->misc->_getClassName()}/deactivate/{$list->student_p_id}", '<span class="btn btn-info btn-xs"><i class="fa fa-check"></i> Active</span>') : anchor("{$this->misc->_getClassName()}/activate/{$list->student_p_id}", '<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Inactive</span>'); ?></td>
									<td><?php 
										$_admission = array(
										...
										), $_admission, $list->admission_status);?></td>

									<td><?php if($list->admission_status == 'pending'){?>

										<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Library Blocked</span>
									<?php }else{?>
                                    	<a href="<?php echo site_url("library/{$this->misc->_getClassName()}/book_issue/{$list->student_p_id}"); ?>"class="btn btn-primary btn-xs"><i class="fa fa-reply"></i> Issue Book </a>
                                    <?php }?>
                                    
									</td>
									<td><a href="<?php echo site_url("library/{$this->misc->_getClassName()}/profile/{$list->student_p_id}"); ?>" class="btn btn-primary btn-xs">
											<i class="fa fa-eye"></i>
										</a>
									</td>
								</tr>
