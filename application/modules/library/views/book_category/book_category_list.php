					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									<input type="hidden" name="cntrlName" id="cntrlName" value="library/<?= str_replace('_', '_', $this->misc->_getClassName()); ?>">
									<td>
										<strong><?= '<span class="badge badge-primary">'.htmlspecialchars($list->book_category_p_id,ENT_QUOTES,'UTF-8').'</span><br/>' ?></strong>
									</td>
									<td><?= htmlspecialchars($list->category_name,ENT_QUOTES,'UTF-8') ?></td>
									<td><?= htmlspecialchars($list->description,ENT_QUOTES,'UTF-8') ?></td>

									<td><?php echo ($list->is_active) ? anchor("library/{$this->misc->_getClassName()}/deactivate/{$list->book_category_p_id}", '<span class="btn btn-info btn-xs"><i class="fa fa-check"></i> Active</span>') : anchor("library/{$this->misc->_getClassName()}/activate/{$list->book_category_p_id}", '<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Inactive</span>'); ?>
									</td>
									
									<td>
										<a href="<?php echo site_url("library/{$this->misc->_getClassName()}/edit/{$list->book_category_p_id}"); ?>" class="btn btn-success btn-xs">
											<i class="fa fa-pencil"></i>
										</a>
										<button class="btn btn-xs btn-danger deleteRow" value="<?= $list->book_category_p_id ?>">
