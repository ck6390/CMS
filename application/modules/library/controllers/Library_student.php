...
class Library_student extends Base_Controller
{
	...
	public function book_issue($id)
	{
		...
		if($this->form_validation->run() == false) {
			...
			if($this->mdl_book_issue->insert($formData) == true) {
				...
				$this->mdl_book->update_book_stock($formData1, $bookID); 
	...
	public function book_return($student_id, $book_id, $book_issue_id)
	{
		...
		$stock = $this->mdl_book->get_book_stock($book_id)->stock;
		//print_r($stock);exit;
		$book = array(
			'stock' => $stock + '1',
		);
		if($this->mdl_book_issue->update($bookReturn, $book_issue_id) == true && $this->mdl_book->update_book_stock($book, $book_id) == true) {
	...
	public function mannual_issue($id)
	{
		...
		if($this->form_validation->run() == false) {
			...
			if ($this->mdl_student->checkAdmissionStatus($id) == true){
			...
			if ($this->mdl_book_issue->checkBookLimit($id) == true){
