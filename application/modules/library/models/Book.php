...
class Book extends Base_Model
{
	...
	*/
	...
	public function get_student_record_bio($st_id)
    {	
    	$sub_str = substr($st_id, 0, 1);
    	$curr_year = date('Y');
    	$curr_month= date('n');
    	$table = "deviceLogs_".$curr_month."_".$curr_year;
    	$bio = $this->load->database('librarydb', TRUE);
    	$bio->select('*');
	    $bio->from($table);
	    $bio->like('DownloadDate',date('Y-m-d'));
	    $bio->like('UserId',$sub_str);
	   /* $bio->where('Direction','in');*/
	    $bio->where('UserId',$st_id);
	    $bio->where('UserId != 1');
	    $bio->order_by('DeviceLogId', 'DESC');/// for out time
	    return $query = $bio->get()->row();
    }
    ...
    public function get_book_stock($book_id)
    {
    	$this->db->select('stock');
    	$this->db->from('books');
    	$this->db->where('accession_no',$book_id);
    	$query = $this->db->get();
		return $query->row();
    }

    public function update_book_stock($formData1,$bookID){
    	//var_dump($bookID);
    	//die;
    	if (!empty($bookID)) {
	        $this->db->where_in('accession_no', $bookID);	       
	        return $this->db->update('books',$formData1);	        
	    }
    }
}
