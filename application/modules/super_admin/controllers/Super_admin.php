...

{
...

	{
		...
		$this->load->model('super_admin/Emp_appraisal', 'emp_appraisal');

		$this->load->model('academics/lecture', 'mdl_lecture');
		$this->load->model('accountants/accountant', 'mdl_accountant');
	...
	{
		$search_item = $this->input->post('search');
		//$search = $this->mdl_student->searchStudent($search_item);
		//var_dump($search);
		//die();		
		if ($search = $this->mdl_student->searchStudent($search_item)){
	
		$id = $search->student_p_id;
			redirect('super_admin/student_profile/'.$id, 'refresh');
		}else{
			$this->session->set_flashdata('error', "Error, Please enter correct faculty id!");
			redirect('super_admin', 'refresh');
		}
...
public function student_profile($id)
	{
		...
		$this->template->load('template', 'contents', 'super_admin/student_profile',$data);
	}
	...
	public function evolution($id, $emp_appraisal_id = null)

	{
		...
		$data['clients_appraisal_data'] = '';

		if(!empty($emp_appraisal_id)){

			$data['clients_appraisal_data'] = $this->emp_appraisal->get($emp_appraisal_id);
		...
		if($this->form_validation->run() == false) {
			...
			if(!empty($cleanPost['end-date'])){
				...
				$formData = array(
					...
					'lacture_date >=' => date("Y-m-d", strtotime($cleanPost['start-date'])),

					'lacture_date <=' => date("Y-m-d", strtotime($cleanPost['end-date']))
...

	{
		...
		if($this->form_validation->run() == false) {
			...
			/*var_dump($data['lists']['fk_semester_id']);
			...
			foreach ($lists as $key => $value) {
				...
				$branch = $value->fk_branch_id;

				

				$formData = array(					

					'fk_semester_id' => $value->fk_semester_id,
				...
				$data['total_lecture'] = $this->mdl_super_Admin->student_attandance_report($formData);

				//var_dump($data['total_lecture']);
				...
				$lectureDate[] = $result[$value->lecture_p_id]['date'];
			...
			$lectureArray = array_unique($lectureDate);
			...
			if(!empty($result)){
				...
				$data['report'] = array(
					...
					'workingDays' => count(array_unique($lectureDate)),

					'startDate' => array_unique($lectureDate)[0],

					'endDate' => end($lectureArray)
	...
	/* Assign paid Leave to employee from super admin*/ 

	public function assign_leave($id)

	{
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->load('template', 'contents', 'assign_employee_leave', $data);
	...
	public function emp_leave_history_status($id)

	{
		...
		if($this->mdl_emp_attendance->update_status($emp_id,$is_active,$lev_id) == true) {
		...
		redirect('super_admin/emp_leave_history/'.$emp_id, 'refresh');		
	...
	public function save_evolution(){
		...
		$this->form_validation->set_rules('casual_leave', 'casual_leave', 'required|trim');

		$this->form_validation->set_rules('leave_without_pay', 'leave_without_pay', 'required|trim');

		$this->form_validation->set_rules('experience', 'experience', 'required|trim');

		$this->form_validation->set_rules('total_points', 'total_points', 'required|trim');
		...
		if($this->form_validation->run() == false) {
			...
			$formData = array(
				...
				'emp_id' => $cleanPost['emp_id'],

				'tech_skill' => $cleanPost['tech_skill'], 

				'avg_stu_sub_sem' => $cleanPost['avg_stu_sub_sem'],

				'inv_ext_act' => $cleanPost['inv_ext_act'],

				'dedi_towadrs_insti' => $cleanPost['dedi_towadrs_insti'],

				'stud_fac_rel' => $cleanPost['stud_fac_rel'] ? $cleanPost['stud_fac_rel'] : "",

				'moral_behaviors' => $cleanPost['moral_behaviors'] ? $cleanPost['moral_behaviors'] : "",

				'no_of_stud_in_class' => $cleanPost['no_of_stud_in_class'] ? $cleanPost['no_of_stud_in_class'] : "",

				'casual_leave' => $cleanPost['casual_leave'] ? $cleanPost['casual_leave'] : "",

				'leave_without_pay' => $cleanPost['leave_without_pay'] ? $cleanPost['leave_without_pay'] : "",

				'experience' => $cleanPost['experience'] ? $cleanPost['experience'] : "",

				'total_points' => $cleanPost['total_points'] ? $cleanPost['total_points'] : "",



			);

			if(!empty($cleanPost['emp_appraisal_id'])){

				if($this->emp_appraisal->update($formData, $cleanPost['emp_appraisal_id']) == true)

				{	

					$this->session->set_flashdata('success', 'Success, Appraisal Successfully Updated');
...

				{
					...
					$this->session->set_flashdata('error', 'Error, Can\'t update Appraisal.');

				}

			}else{

				if($this->emp_appraisal->insert($formData) == true)

				{	

					$this->session->set_flashdata('success', 'Success, Appraisal Successfully Create');
			...
			redirect("super_admin/appraisal", 'refresh');
...

	{
		...
		$this->template->set('title', 'Employee Appraisal');

		$data['lists'] = $this->emp_appraisal->get_all();

		$this->template->load('template', 'contents', 'super_admin/appraisal_list',$data);

	}

	public function evolution_view($emp_id,$id)

	{

		$this->template->set('title', 'Employee Appraisal View Print');

		$data['info'] = $this->mdl_employee->get($emp_id);

		$data['lists'] = $this->mdl_super_Admin->emp_all_lecture($emp_id);

		$data['clients_appraisal'] = $this->emp_appraisal->get($id);

		$data['id'] = $id;

		$this->template->load('template', 'contents', 'super_admin/emp_evolution_view',$data);
