...
class Super_admin extends Base_Controller
{
	...
	public function student_attandance()
	{
		...
		$this->form_validation->set_rules('studentid', 'semester', 'required|trim');
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->set('title', 'Student Attandance Branch Wise');
			$data['lists'] = $lists = $this->mdl_super_Admin->student_attandance_date($cleanPost['studentid']);
			// echo "<PRE>";
			// print_r($data['lists']);
			foreach ($lists as $key => $value) {
				$att = json_decode($value->student_attandance, true);
				foreach ($att as $at) {
					if($cleanPost['studentid'] == $at['student_id']){
						$student_id = $at['student_id'];
						$att_status = $at['attance_status'];
					}
				}
				$result[$value->lecture_p_id]['date'] = $value->created_on; 
