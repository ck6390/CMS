						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									...
									<td><?= '<strong> '.htmlspecialchars($list->fee_group_name,ENT_QUOTES,'UTF-8').'</strong>'; ?></td>
									<td><?php echo ($list->is_active) ? anchor("accounting/{$this->misc->_getClassName()}/deactivate/".$list->fee_group_p_id, '<span class="btn btn-info btn-xs"><i class="fa fa-check"></i> Active</span>') : anchor("accounting/{$this->misc->_getClassName()}/activate/". $list->fee_group_p_id, '<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Inactive</span>'); ?>
									</td>									
									<td>
										<a href="<?php echo site_url("accounting/".$this->misc->_getClassName()."/edit/".$list->fee_group_p_id); ?>" class="btn btn-success btn-xs">
											<i class="fa fa-pencil"></i>
										</a>
										<!-- <button class="btn btn-xs btn-danger deleteRow" value="<?= $list->fee_group_p_id ?>">
