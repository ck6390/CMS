...
class Books extends Base_Controller
{
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('quantity', 'Book Quantity', 'required|trim|regex_match[/^[0-9]{4}$/]');
		$this->form_validation->set_rules('book-category', 'Book Category', 'required|trim');
		...
		if($this->form_validation->run() == false) {
			$data['lastId'] = $this->mdl_book->get_next_id();
			$this->template->set('title', 'Add Book');
			$this->template->load('template', 'contents', 'book/book_add', $data);
