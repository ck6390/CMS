						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								foreach ($lists as $list) { $i++; 
									$branch_id = $this->mdl_student->get($list->student_id)->fk_branch_id;
									?>
								<tr>
									<input type="hidden" name="cntrlName" id="cntrlName" value="<?= $this->misc->_getClassName(); ?>">
									<td><span class="badge badge-danger"><?= "{$i}." ?></span></td>
									<td><?= '<strong> '.htmlspecialchars($this->mdl_student->get($list->student_id)->student_unique_id,ENT_QUOTES,'UTF-8').'</strong>'; ?></td>
									...
									<td><?= '<strong> '.htmlspecialchars($list->sms_type,ENT_QUOTES,'UTF-8').'</strong>'; ?></td>
