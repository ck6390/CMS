...
class Sms_settings extends Base_Controller
{
	/**
	...
	function __construct()
	{
		...
		parent::__construct();
		$this->load->model('sms_setting', 'mdl_sms_setting');
	}
	...
	public function index()
	{
		...
		$this->form_validation->set_rules('auth-key', 'auth key', 'trim');
		$this->form_validation->set_rules('sender-id', 'sms sender id', 'trim');
		$check_data = $this->mdl_sms_setting->check_row();
		if($this->form_validation->run() == false) {	
			//$id = $this->db->insert_id();
			
			$data['info'] = $this->mdl_sms_setting->get($check_data);
			
			$this->template->set('title', 'Sms Setting');
			$this->template->load('template', 'contents', 'sms_setting/add', $data);
		} else {
			...
			$formData = array(
				...
				'auth_key' => $cleanPost['auth-key'],
				'sender_id' => $cleanPost['sender-id'],
				'is_active' => $cleanPost['status']
				
			);

			if(!empty($check_data)){
				$this->mdl_sms_setting->update($formData,$check_data);
			}else{
				$this->mdl_sms_setting->insert($formData);
			}

			redirect('setting/sms_settings', 'refresh');
		}	
	}
