						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									...
									<td>
										<strong><?= htmlspecialchars($list->purpose_name,ENT_QUOTES,'UTF-8') ?></strong>
									</td>
									<td><?= htmlspecialchars($list->description,ENT_QUOTES,'UTF-8') ?></td>
									<td><?= $list->is_active ? anchor("setting/{$this->misc->_getClassName()}/deactivate/" . $list->debit_purpose_p_id, '<span class="btn btn-info btn-xs"><i class="fa fa-check"></i> Active</span>') : anchor("setting/{$this->misc->_getClassName()}/activate/" . $list->debit_purpose_p_id, '<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Inactive</span>'); ?>
									</td>
									<td>
										<a href="<?php echo site_url("setting/{$this->misc->_getClassName()}/edit/{$list->debit_purpose_p_id}"); ?>" class="btn btn-success btn-xs">
											<i class="fa fa-pencil"></i>
										</a>
										<?php if($this->auth->_isDeveloper()) { ?>
											<button class="btn btn-danger btn-xs deleteRow" value="<?= $list->debit_purpose_p_id ?>">
