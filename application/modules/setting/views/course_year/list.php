						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									...
									<td><?php echo ($list->is_active) ? anchor("setting/{$this->misc->_getClassName()}/deactivate/{$list->course_year_p_id}", '<span class="btn btn-info btn-xs"><i class="fa fa-check"></i> Active</span>') : anchor("setting/{$this->misc->_getClassName()}/activate/{$list->course_year_p_id}", '<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Inactive</span>'); ?>
									</td>
									
									<td>
										<a href="<?php echo site_url("setting/".$this->misc->_getClassName()."/edit/".$list->course_year_p_id); ?>" class="btn btn-success btn-xs">
											<i class="fa fa-pencil"></i>
										</a>
										<button class="btn btn-xs btn-danger deleteRow" value="<?= $list->course_year_p_id ?>">
											<i class="fa fa-trash"></i>
										</button>
										<?php if($this->auth->_isDeveloper()) { ?>
											<button class="btn btn-danger btn-xs deleteRow" value="<?= $list->course_year_p_id ?>">
												<i class="fa fa-trash"></i>
											</button> 
										<?php } ?>
									</td>
