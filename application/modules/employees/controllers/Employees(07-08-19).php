...
class Employees extends Base_Controller
{
	...
	public function edit_job_details($id)
	{
		...
		if($this->form_validation->run() == false) {
			...
			$formData = array(
				...
				'emp_login_time' => $cleanPost['in-time'],
				'emp_logout_time' => $cleanPost['out-time'],
				'allowance' => $cleanPost['allowance'] ? $cleanPost['allowance'] : "",
				'emp_salary' => $cleanPost['salary'],
				'emp_resident_type' => $cleanPost['resident-type'],
	...
	public function edit_login_details($id)
	{
		...
		if($this->form_validation->run() == false) {
			...
			$formData = array(
				...
				'view_password' => $cleanPost['display-password'],
				'password' => $hashed
			);
			// update to database
			if($this->mdl_employee->update($formData, $id) == true) {
	...
	public function lecture_student_attadance($employeeId,$lectureId){
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->load('template', 'contents', 'lecture_student_attadance', $data);
		}else{
	...
	public function subject_unit($id)
	{
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->load('template', 'contents', 'emp_subject_unit', $data);
		} else {
			...
			redirect("employees/subject_unit/{$id}", 'refresh');
		}
	}
	...
	public function subject_unit_edit($emp_id,$sub_unit_id)
	{
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->load('template', 'contents', 'emp_subject_unit_edit', $data);
		} else {
			...
			redirect("employees/subject_unit/{$emp_id}", 'refresh');
		}
	}
