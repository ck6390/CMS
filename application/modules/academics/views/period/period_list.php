						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									...
									<td><?= '<span class="badge badge-primary">'.htmlspecialchars($list->period_p_id,ENT_QUOTES,'UTF-8').'</span><br/>' ?></td>
									<td><?= '<strong> '.htmlspecialchars($list->period_name,ENT_QUOTES,'UTF-8').'</strong>'; ?></td>
									
									<td><?= '<strong> '.htmlspecialchars($list->period_desc,ENT_QUOTES,'UTF-8').'</strong>'; ?></td>

									<td><?php echo ($list->is_active) ? anchor("academics/{$this->misc->_getClassName()}/deactivate/{$list->period_p_id}", '<span class="btn btn-info btn-xs"><i class="fa fa-check"></i> Active</span>') : anchor("academics/{$this->misc->_getClassName()}/activate/{$list->period_p_id}", '<span class="btn btn-xs btn-warning"><i class="fa fa-ban"></i> Inactive</span>'); ?>
									</td>
									
									<td>
										<a href="<?php echo site_url("academics/{$this->misc->_getClassName()}/edit/".$list->period_p_id); ?>" class="btn btn-success btn-xs">
