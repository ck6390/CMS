...
class Temporary_lectures extends Base_Controller
{
	/**
	...
	{	
		$data['lists'] = $this->mdl_lecture->get_all_temprory_lecture();
		$this->template->set('title', 'Lecture List');
		$this->template->load('template', 'contents', 'temporary_lectures/temp_lecture_list', $data);
	}
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('engaged-of', 'engaged of faculty', 'required|trim');
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->load('template', 'contents', 'temporary_lectures/temp_lecture_add');
		} else {
			$post = $this->input->post(null, true);
			$cleanPost = $this->security->xss_clean($post);
			$formData = array(
				'employee_id' => $cleanPost['employee-id'],
				'engaged_of_faculty' => $cleanPost['engaged-of'],
				'employee_id' => $cleanPost['employee-id'],
			...
			redirect('academics/temporary_lectures', 'refresh');
		}
	}
	...
	public function edit($id) 
	{
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->load('template', 'contents', 'temporary_lectures/temp_lecture_edit', $data);
		} else {
			$post = $this->input->post(null, true);
			$cleanPost = $this->security->xss_clean($post);
			$formData = array(
