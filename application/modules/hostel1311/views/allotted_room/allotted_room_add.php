					...
					if(dataObj) {
						$(dataObj).each(function() {
							...
							$('select[name="room-id"]').append(option);
								$('#roomList').append('<div class="col-md-4"><div class="ibox"><div class="ibox-content text-center product-box '+(availableBed==0 ? 'bg-danger' :'bg-primary')+'"><div class="product-desc">'+this.room_number+' <i class="fa fa-long-arrow-right"></i> '+ availableBed +'</div></div></div></div>');
