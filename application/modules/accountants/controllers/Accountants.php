...
class Accountants extends Base_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('setting/branch','branch');
	...
	{	
		//var_dump($id);
		/*$student = $this->mdl_student->get_fields($id,array('is_restericated'));
		...
		/*}else{
			$filter = explode(",", $student->is_restericated);
			$error = "";
			for ($i=0; $i < count($filter); $i++) { 
				$resterication_lists= get_restrication($filter[$i]);
				foreach ($resterication_lists as $value) {	
					$error .= "<li>".$value->restriction." (".$value->description.")</li>";
				}				
			}
			$this->session->set_flashdata('danger',$error);

			redirect('accountants/student_profile/'.$id);
		}*/
	...
	{	
		...
		if($this->form_validation->run() == false) {
			...
			$this->template->load('template', 'contents', 'accountants/day_statement',$data);
		}else {
			$post = $this->input->post(null, true);
			$cleanPost = $this->security->xss_clean($post);
			//var_dump($cleanPost);
			//die;
			$acount_type = explode(',', $_POST['account_type']);
			if($cleanPost['day_due_fee'] == "hostel"){

				$data['fee'] = $this->mdl_accountant->hostel_fee_between_dates($_POST["month-from"],$_POST["month-to"],$acount_type[0]);
				
			}elseif($cleanPost['day_due_fee'] == "fooding"){

				$data['fee'] = $this->mdl_accountant->fooding_fee_between_dates($_POST["month-from"],$_POST["month-to"],$acount_type[0]);

			}elseif($cleanPost['day_due_fee'] == "library"){

				$data['library_fee'] = $this->mdl_accountant->library_fee_between_dates($_POST["month-from"],$_POST["month-to"],$acount_type[0]);
			}elseif($cleanPost['day_due_fee'] == "academic_fee"){

				$data['academic_fee'] = $this->mdl_accountant->academic_fee_between_dates($_POST["month-from"],$_POST["month-to"],$acount_type[0]);

			}elseif($cleanPost['day_due_fee'] == "all"){
				$data['lists'] = $this->mdl_accountant->accountant_day_statement_between_dates($_POST["month-from"],$_POST["month-to"],$acount_type[0]);

			}
			$data['account_type'] = $acount_type[1];
			$this->template->set('title', 'Student Payment');
			$this->template->load('template', 'contents', 'accountants/day_statement',$data);
		}
	}
	...
	{	
		...
		if($this->form_validation->run() == false) {
			...
			$cleanPost = $this->security->xss_clean($post);
			$acount_type = explode(',', $_POST['account_type']);
			$data['lists'] = $this->mdl_accountant->accountant_debit_statement_between_dates($_POST["purpose"],$_POST["month-from"],$_POST["month-to"],$acount_type[0]);
			$this->template->set('title', 'Student Payment');
			$data['account_type']=$acount_type[1];			
			$this->template->load('template', 'contents', 'accountants/debit_statement',$data);
		}
	...
	public function payment_sms($id)
	{
		//echo $this->uri->segment('3');
		//die;
		$data['title'] = "Print Invoice";
		$data['payment_invoice'] = $this->mdl_accountant->payment_invoice_student($id);	
		//var_dump($data['payment_invoice']->fk_semester_id);
		
		//$feeType = $data['payment_invoice']->fee_types_id;
		$studentUniqueId = $data['payment_invoice']->student_unique_id;
		$studentId =  $data['payment_invoice']->student_id;
		$student_name =  $data['payment_invoice']->student_full_name;
		$amount =  $data['payment_invoice']->paid_amount;
		$transcationNumber =  $data['payment_invoice']->payment_id;
		$phoneNumber =  $this->mdl_student->get($studentId)->student_sms_no;
		$feeTypes = explode(",", $data['payment_invoice']->fee_types_id);
		$paymentDate = date_create($data['payment_invoice']->created_on);		
		...
		foreach($feeTypes as $ftps){
			...
			if($result == "ac"){
				...
				$feeName = $this->mdl_fee_type->get($academic_fees->fk_fee_type_id)->fee_type_name." - ".$academic_fees->fee_amount."  Rs.";
				...
				$feeName = $this->mdl_fee_type->get($hostel_charge->fk_fee_type_id)->fee_type_name." ".date('F Y', strtotime($hostel_charge->hostel_charge_month));
				...
				$feeName = $this->mdl_fee_type->get($library_fees->fine_type_id)->fee_type_name." - ".$list->paid_amount."  Rs.";
			}
			elseif($result =="gr"){

				$result1 = substr($ftps, 2, $length);
				
				$feeName = $this->mdl_fee_type->get($result1)->fee_type_name." - ".$this->mdl_fee_type->get($result1)->fee_type_amount."  Rs.";	

			}elseif($result =="ot"){
				//var_dump($data['payment_invoice']->fk_semester_id);
				if(!empty($data['payment_invoice']->fk_semester_id)){
					$feeName = $data['payment_invoice']->other_fee." - ".   $this->mdl_semester->get($data['payment_invoice']->fk_semester_id)->semester_name;
				}else{

					$feeName = $data['payment_invoice']->other_fee ;
				}
	        }
		}
		//die();
		//echo $feeName;
		//die();
		$message =$student_name." ( ".$studentUniqueId." ) receive your payment Rs. ".$amount." against of ".$feeName." dated on ".date_format($paymentDate,"d-m-Y")." Thanks. GMCP";
		$urlencode = urlencode($message);  
		//var_dump($message);
		//die(); 
		send_sms($phoneNumber,$urlencode);
		//var_dump($phoneNumber);
		//var_dump();
		//die(); 
		
		if($this->uri->segment('4') != 0){  
			$this->session->set_flashdata('success', "Success, message send successfully!");
			redirect('accountants/day_statement');

		}else{			
			$this->session->set_flashdata('success', "Success, message send successfully!");
			//var_dump($this->session->flashdata('success'));	
			//die();
			redirect('accountants/payment_invoice/'.$this->uri->segment('3'));
		}
	}
}
