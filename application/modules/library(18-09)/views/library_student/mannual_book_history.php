						...
						<table class="table table-striped table-bordered table-hover dataTablesView">
							...
							<?php
								...
								<tr>
									...
									<td><?= htmlspecialchars($list->book_call_no,ENT_QUOTES,'UTF-8') ?></td>

									<td><?php echo $this->mdl_book->get_single_value_by_id('book',$list->book_id,'book_name');?></td>
                                    ...
                                    <td><?php if($list->is_active == 0){?>
                                    	...
                                    	<a href="<?php echo site_url("library/{$this->misc->_getClassName()}/mannual_book_return/{$info->student_p_id}/{$list->book_id}/{$list->book_issue_p_id}"); ?>"class="btn btn-primary btn-xs"><i class="fa fa-undo"></i> Return </a>
